...
int autonRecorderMaxNumberMenus = 2;
...
void lcd(){
  lcdMainMenu();
    ...
    break;
    case 5:
      lcdProgrammingSkills();// select prog skills
    break;
    case 6:
      lcdAutonRecorder();
    break;
...
void lcdProgrammingSkills(){
  if ((lcdReadButtons(uart1) == LCD_BTN_CENTER && previousLCD == LCD_BTN_CENTER) || menuStay == LCD_BTN_CENTER){
    ...
    if ((lcdReadButtons(uart1) == LCD_BTN_RIGHT && previousLCD == LCD_BTN_RIGHT) || endResultMenu == LCD_BTN_RIGHT){
      ...
      lcdSetText(uart1, 2, "is selected");
      progSkills++;
      if (lcdReadButtons(uart1) == LCD_BTN_CENTER && previousLCD != LCD_BTN_CENTER){
        menuCount = 0;
        menuStay = 0;
  ...
  if ((lcdReadButtons(uart1) == LCD_BTN_CENTER && previousLCD == LCD_BTN_CENTER) || menuStay == LCD_BTN_CENTER){
    ...
    switch (sensorMenuCount) {
      case 0:
        ...
        snprintf(encoderAve, 17, "%d", (encoderGet(frontLeftEncoder) + encoderGet(frontRightEncoder))/2 );
      ...
      case 2:
        ...
        snprintf(pincePotLCD, 17, "%d", analogRead(pince_pot));
...
void lcdMainMenu(){// code for main menu in the lcd
  ...
  switch(menuCount){
    case 0:
      lcdClear(uart1);
      lcdSetText(uart1, 1, "Cash me outside");
      lcdSetText(uart1, 2 , "How bow dah?");
      subMenuCount = 0;
      break;
    case 1:
      lcdClear(uart1);
    ...
    case 5:
      lcdClear(uart1);
      lcdSetText(uart1, 1, "Programming");
      lcdSetText(uart1, 2, "Skills");
      subMenuCount = 5;
      break;
    case 6:
      lcdClear(uart1);
      lcdSetText(uart1, 1, "Auton");
      lcdSetText(uart1, 2, "Recording");
      subMenuCount = 6;
  }
}
